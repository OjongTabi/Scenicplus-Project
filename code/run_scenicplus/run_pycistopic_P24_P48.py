...
################################################ subset cistopic object for two developmental stages

stages = ['P24', 'P48']
names = cell_data['AnnotatedJul24'].unique()

combined_names = [f"{stage}_{name}" for name in names for stage in stages]
...
cistopic_obj.cell_data = cistopic_obj.cell_data[cistopic_obj.cell_data['Annotation_celltype_stage'].isin(combined_names)]

# Get the filtered cell names
filtered_cell_names = cistopic_obj.cell_data.index

# Get the indices of these filtered cell names in the 'cell_names' list 
filtered_cell_indices = np.where(np.isin(cistopic_obj.cell_names, filtered_cell_names))[0]

# Subset the fragment_matrix and binary_matrix by these column indices 
cistopic_obj.fragment_matrix = cistopic_obj.fragment_matrix[:, filtered_cell_indices]
cistopic_obj.binary_matrix = cistopic_obj.binary_matrix[:, filtered_cell_indices]
